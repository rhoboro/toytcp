...
const UNDETEMINED_IP_ADDR: std::net::Ipv4Addr = Ipv4Addr::new(0, 0, 0, 0);
const UNDETEMINED_PORT: u16 = 0;
...
impl TCP {
    ...
    pub fn recv(&self, sock_id: SockID, buffer: &mut [u8]) -> Result<usize> {
        ...
        while received_size == 0 {
            ...
            self.wait_event(sock_id, TCPEventKind::DataArrive);
            table = self.sockets.write().unwrap();
            socket = table
                .get_mut(&sock_id)
                .context(format!("no such socket: {:?}", sock_id))?;
    ...
    fn process_payload(&self, socket: &mut Socket, packet: &TCPPacket) -> Result<()> {
        ...
        self.publish_event(socket.get_sock_id(), TCPEventKind::DataArrive);
        Ok(())
    }
    ...
    fn listen_handler(
        ...
        if packet.get_flag() & tcpflags::SYN > 0 {
            let mut connection_socket = Socket::new(
                // 接続済みソケットを生成し、Listen ソケットはずっと Listen し続ける
